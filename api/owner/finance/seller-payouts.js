// api/owner/finance/seller-payouts.js
...
export default async function handler(request) {
  ...
  if (request.method === 'GET') {
    try {
      ...
      const payoutsSelect = `*, sellers!inner(seller_id,shop_name,owner_name,email,mobile,upi_id,commission_rate)`;
      let payoutsUrl = `${supabaseUrl}/rest/v1/payouts?select=${encodeURIComponent(payoutsSelect)}&user_type=eq.seller&order=${sort_by}.${sort_order}`;
      ...
      if (seller_id) {
        payoutsUrl += `&user_id=eq.${seller_id}`;
      }
      ...
      const allPayoutsUrl = `${supabaseUrl}/rest/v1/payouts?select=amount,status,requested_at&user_type=eq.seller`;
      ...
      // Get total wallet balance of all sellers
      const sellersUrl = `${supabaseUrl}/rest/v1/sellers?select=wallet_balance`;
      ...
      const totalWalletBalance = sellers?.reduce((sum, s) => sum + (s.wallet_balance || 0), 0) || 0;
      ...
      return new Response(JSON.stringify({
        ...
        payouts: payouts?.map(p => ({
          ...
          seller: {
            ...
            mobile: p.sellers?.mobile,
            upi_id: p.sellers?.upi_id,
            commission_rate: p.sellers?.commission_rate
          },
          amount: p.amount,
  ...
  if (request.method === 'POST') {
    try {
      ...
      // Get payout details with seller info
      const payoutSelect = `*, sellers!inner(seller_id, shop_name, email, mobile, upi_id, wallet_balance)`;
      ...
      switch (action) {
        ...
        case 'reject':
          ...
          const sellerUrl = `${supabaseUrl}/rest/v1/sellers?select=wallet_balance&seller_id=eq.${payout.user_id}`;
          ...
          const currentBalance = sellerData[0]?.wallet_balance || 0;

          // Refund amount back to seller wallet
          await fetch(`${supabaseUrl}/rest/v1/sellers?seller_id=eq.${payout.user_id}`, {
            method: 'PATCH',
            headers: {
              'apikey': supabaseKey,
              'Authorization': `Bearer ${supabaseKey}`,
        ...
        case 'fail':
          ...
          const sellerBalanceUrl = `${supabaseUrl}/rest/v1/sellers?select=wallet_balance&seller_id=eq.${payout.user_id}`;
          ...
          const currentSellerBalance = sellerBalanceData[0]?.wallet_balance || 0;
          ...
          await fetch(`${supabaseUrl}/rest/v1/sellers?seller_id=eq.${payout.user_id}`, {
            ...
            body: JSON.stringify({ wallet_balance: currentSellerBalance + payout.amount })
          });
  ...
  if (request.method === 'PUT') {
    try {
      ...
      for (const payout_id of payout_ids) {
        try {
          const payoutUrl = `${supabaseUrl}/rest/v1/payouts?select=*,sellers!inner(seller_id,shop_name)&payout_id=eq.${payout_id}`;
          const payoutResponse = await fetch(payoutUrl, {
            headers: {
              'apikey': supabaseKey,
              'Authorization': `Bearer ${supabaseKey}`,
